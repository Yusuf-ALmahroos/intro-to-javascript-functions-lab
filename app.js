...
const basicCalculator = (num1, num2, operiation) => 
{
  switch(operiation)
  {
    case 'add':
...
console.log('Exercise 9 Result:', basicCalculator(10, 5, "sqrt"));
...
{
  const firstLetters = firstName.slice(0, 3);
  const lastLetters  = lastName.slice(0, 3);
  const namesLen     = firstName.length + lastName.length;
  
  return `${firstLetters}${lastLetters}${namesLen}`; 
}
